        ...
        for source_path_str in source_paths:
            ...
            for source_file in source_files:
                ...
                for content_path in content_path_set:
                    qb_path_replaced = qb_path.replace(
                        "Downloads_sata", "Downloads/sata"
                    )
                    qb_path_replaced_1 = qb_path.replace(
                        "Downloads_sata", "sata/Download"
                    )
                    if (
                        (qb_path in content_path)
                        or (qb_path_replaced in content_path)
                        or (qb_path_replaced_1 in content_path)
                    ):
                        is_exist = True
                        break
